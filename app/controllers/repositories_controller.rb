class RepositoriesController < ApplicationController
  ...
  def index
    @repository_by_language = Repository.group(:language)
  end

  # GET /repositories/1 or /repositories/1.json
  def show
    @repositories = Repository.where(language: params[:id], category: params[:category])
  end

  def category
    @language = params[:id]
    @repositories = Repository.where(language: params[:id]).group(:category)
  end
  ...
  private
    ...
    def set_repository
      @repository = Repository.where(language: params[:id]).group(:category)
    end

    # Only allow a list of trusted parameters through.
    def repository_params
      params.require(:repository).permit(:language, :category, :public_url)
    end
end
